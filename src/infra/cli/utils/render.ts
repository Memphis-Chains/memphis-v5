...
function generateBashCompletionScript(): string {
  return [
    ...
    '    COMPREPLY=( $(compgen -W "setup configure init health reflect learn insights connections suggest serve providers:health providers models chat ask categorize decide infer agents relationships trust mcp tui doctor onboarding chain sync trade vault embed ascii progress celebrate completion help" -- "${cur}") )',
    '    return 0',
    '  fi',
    '',
    ...
    '    setup|configure|init) flag_candidates="--out --force --json" ;;',
    '    onboarding)',
    '      if [[ "${sub}" == "wizard" ]]; then',
...
function generateFishCompletionScript(): string {
  return [
    ...
    'for c in memphis',
    '  complete -c $c -f -n "__fish_use_subcommand" -a "setup configure init health reflect learn insights connections suggest serve providers:health providers models chat ask categorize decide infer agents relationships trust mcp tui doctor onboarding chain sync trade vault embed completion help"',
    ...
    '  complete -c $c -n "__fish_seen_subcommand_from setup configure init" -l out',
    '  complete -c $c -n "__fish_seen_subcommand_from setup configure init" -l force',
