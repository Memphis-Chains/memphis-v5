...
describe('openclaw plugin package identity', () => {
  it('keeps manifest and package names aligned', () => {
    const pkg = JSON.parse(readFileSync('packages/@memphis/openclaw-plugin/package.json', 'utf8')) as { name: string };
    const manifest = JSON.parse(readFileSync('packages/@memphis/openclaw-plugin/openclaw.plugin.json', 'utf8')) as { id: string };
