...
describe('RelationshipGraph', () => {
  it('stores relationships and computes trust score', () => {
    ...
    registry.register({ did: 'did:memphis:b', name: 'B', publicKey: 'b', capabilities: [], reputation: 80, lastSeen: new Date() });
    ...
    graph.addRelationship({ from: 'did:memphis:a', to: 'did:memphis:b', type: 'collaborates', strength: 0.5, interactions: 2, lastInteraction: new Date() });
  ...
  it('returns collaborators and partner suggestions', () => {
    const dir = mkdtempSync(join(tmpdir(), 'memphis-relationship-'));
    const registry = new AgentRegistry(join(dir, 'agents.json'));
    registry.register({ did: 'did:memphis:a', name: 'A', publicKey: 'a', capabilities: [], reputation: 40, lastSeen: new Date() });
    registry.register({ did: 'did:memphis:b', name: 'B', publicKey: 'b', capabilities: [], reputation: 90, lastSeen: new Date() });
    registry.register({ did: 'did:memphis:c', name: 'C', publicKey: 'c', capabilities: [], reputation: 70, lastSeen: new Date() });

    const graph = new RelationshipGraph(registry, join(dir, 'relationships.json'));
    graph.addRelationship({ from: 'did:memphis:a', to: 'did:memphis:b', type: 'collaborates', strength: 0.7, interactions: 2, lastInteraction: new Date() });
    graph.addRelationship({ from: 'did:memphis:c', to: 'did:memphis:a', type: 'trusts', strength: 0.4, interactions: 1, lastInteraction: new Date() });
