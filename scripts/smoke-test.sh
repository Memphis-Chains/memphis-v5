...
set +e
DEFAULT_PROVIDER=local-fallback node --import tsx src/infra/cli/index.ts doctor --json >/tmp/mv4-smoke-doctor.json
DOCTOR_EXIT=$?
set -e
...
DEFAULT_PROVIDER=local-fallback node --import tsx src/infra/cli/index.ts health --json >/tmp/mv4-smoke-health.json
DEFAULT_PROVIDER=local-fallback node --import tsx src/infra/cli/index.ts ask --input "smoke" --json >/tmp/mv4-smoke-ask.json
